...
/**
 * shock diffraction的边界条件，reflective on top and bottom, transmissive on left and right
 * @tparam T 肯定是个Equ类型，这里特定是MHD的8个量
 * @param uPlus1
 * @param NUM_GROW
 */
template <class T>
void customBoundary3(vector<vector<T>> &uPlus1, int NUM_GROW) {
    int nx = uPlus1.size();
    int ny = uPlus1[0].size();
    ...
    for (int j = 0; j < ny; j++) {
        for (int l = 0; l < NUM_GROW; l++) {
            // 左边界透射：将最左边内部值复制到左边界区域
            uPlus1[l][j] = uPlus1[NUM_GROW][j];
            // 右边界透射：将最右边内部值复制到右边界区域
            uPlus1[nx - NUM_GROW + l][j] = uPlus1[nx - NUM_GROW - 1][j];
        }
    }
    ...
    for (int i = 0; i < nx; i++) {
        for (int l = 0; l < NUM_GROW; l++) {
            ...
            uPlus1[i][NUM_GROW - l - 1][2] *= -1; // 反转vy
            uPlus1[i][NUM_GROW - l - 1][6] *= -1; // 反转by

            // 反射性边界条件，y上端
            uPlus1[i][ny - NUM_GROW + l] = uPlus1[i][ny - NUM_GROW - 1 - l];

            uPlus1[i][ny - NUM_GROW + l][2] *= -1; // 反转vy
            uPlus1[i][ny - NUM_GROW + l][6] *= -1; // 反转by
        }
    }
}

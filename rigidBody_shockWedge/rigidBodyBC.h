...
void getWeight(vector<vector<Equ>> &U,int i,int j){
...
//        cout<<"Point is "<<i<<' '<<j<<" D= "<<endl<<A_Dirichlet<<endl;
//        cout<<"Point is "<<i<<' '<<j<<" N= "<<endl<<A_Neumann<<endl;
        ...
        for (int k=0;k<8;k++){
            if (k==1||k==5) {
                vector<tuple<double,double*>> UxD;UxD.push_back(make_tuple(1,&valueDBC[k]));
                vector<tuple<double,double*>> UyD;UyD.push_back(make_tuple(1,&valueDBC[k+1]));
        ...
        for (int k=0;k<8;k++){
            if (k==1||k==5){
                ...
                vector<tuple<double,double*>> Ux;Ux.push_back(make_tuple(1,&valueDBC[k]));
                vector<tuple<double,double*>> Uy;Uy.push_back(make_tuple(1,&valueDBC[k+1]));
 ...
 */
void rigidBodyPhi(vector<vector<Equ>> &U){
    ...
    for (int i = NUM_GROW; i < x_nCells + NUM_GROW; i++) {
        for (int j = NUM_GROW; j < y_nCells + NUM_GROW; j++) {
            if (  get<0>(Phi[i][j])<=0  &&  get<0>(Phi[i][j])>=(-max(dx,dy)*(NUM_GROW+3))  )  getWeight(U,i,j);
        }
    }
//    getWeight(U,28,68);
}
 ...
 * @param U Into U. Can be different to the bound one.
 */
void rigidBodyBC(vector<vector<Equ>> &U){
    //weight的来源已经被记录下来了，这里的U可以不一样
    double dx = SimMesh.dx;
    double dy = SimMesh.dy;
    int x_nCells=SimMesh.x_nCells;
    int y_nCells=SimMesh.y_nCells;

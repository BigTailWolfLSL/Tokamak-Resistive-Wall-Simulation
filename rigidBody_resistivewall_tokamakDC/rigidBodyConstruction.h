...
int withinLineSegment(BasicLine* e,tuple<double,double> P1,tuple<double,double> P2,double xt,double yt){
    ...
    if ((a*xt+b*yt+c)<0) {return -2;}////利用强大的弥合机制 in rigidBodyBC，直接设置为0就完事了
    else if ((a1*xt+b1*yt+c1)<0) {return -1;}
    else if ((a2*xt+b2*yt+c2)<0) {return 1;}
    else return 0;
    ////返回-1为p1左侧并且e为正，返回1为p2右侧并且e为正，0则是在线段对应空间上,-2则是e为负
}
...
class BasicConvexPolygon:public Component{
public:
    vector<BasicLine*> e;
    vector<double> coorx,coory;
    bool inside;
    BasicConvexPolygon(vector<double> const & xt,vector<double> const & yt,bool insideOrFalse):inside(insideOrFalse){
        if (xt.size()!=yt.size()) {
            cout<<"Check input points."<<endl;
            exit(-1);
        }
        int n=xt.size();
        for (int i=0;i<n;i++){
            coorx.push_back(xt[i]);
            coory.push_back(yt[i]);
        }
        ...
        for (int i=0;i<n;i++){
            ...
            //            if (dist_it*dist_jt<0){
            //                cout<<"Polygon points sequence is wrongly set. Or it is not a convex polygon."<<endl;
            //                exit(-1);
            //            }
            //
            //            if (dist_jt>0||dist_it>0){
            ...
            e.push_back(new BasicLine(a,b,c));
        }
    }
    ...
    std::tuple<double,Normal> operate(double x, double y) override {
        ...
        int jt=it+1;if (jt==n) jt=0;
        int index= withinLineSegment(e[it], make_tuple(coorx[it],coory[it]), make_tuple(coorx[jt],coory[jt]),x,y);
        if (index==-1){
            //p1扇形范围内
            BasicCircle cir(coorx[it],coory[it],0,false);
            auto result=cir.operate(x,y);
            T=get<0>(result);
            NT=get<1>(result);
        } else if (index==1) {
            //p2扇形范围内
            BasicCircle cir(coorx[jt],coory[jt],0,false);
            auto result=cir.operate(x,y);
            T=get<0>(result);
            NT=get<1>(result);
        }

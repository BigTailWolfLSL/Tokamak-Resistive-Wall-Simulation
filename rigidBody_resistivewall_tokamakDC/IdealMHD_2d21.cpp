...
tuple<double,double> integratetotalenergy(vector<vector<Equ>> &u,vector<vector<Equ>> &uRigidBody){
    ...
    for (int i=NUM_GROW;i<x_ncells+NUM_GROW;i++){
        for (int j=NUM_GROW;j<y_ncells+NUM_GROW;j++){
            if (get<0>(Phi[i][j])<=0){
                ansglobal += 0.5*dx*dy*(uRigidBody[i][j].B_x*uRigidBody[i][j].B_x
                            +uRigidBody[i][j].B_y*uRigidBody[i][j].B_y
                            +uRigidBody[i][j].B_z*uRigidBody[i][j].B_z);
            } else {
                anslocal += u[i][j].energy * dx * dy;
                ansglobal += u[i][j].energy * dx * dy;
            }
        }
    }
    return make_tuple(anslocal,ansglobal);
}
...
int main() {
    ...
    initiateDC();//TODO: divergence cleaning
    double eta_wall=5.6e-8;
    ...
    double t = tStart;
    vector<tuple<double,double>> inteTotalEnergy;inteTotalEnergy.push_back(integratetotalenergy(u, uRigidBody));
    // 初始化结束
    ...
    while (t < tStop) {
        ...
        DivergencdCleaning(u,dt);//TODO: divergence cleaning
        ...
        if (t==checkpointStep*checkpointi&&t!=tStop){
            string outfile1="/CFD_file/Tokamak_application/tungstenFirstDC/tokamak_t";
            ...
            inteTotalEnergy.push_back(integratetotalenergy(u,uRigidBody));
            for (int i = NUM_GROW; i < x_nCells + NUM_GROW; i++) {
                for (int j = NUM_GROW; j < y_nCells + NUM_GROW; j++) {
                    double x = x0 + (i + 0.5 - NUM_GROW) * dx;
                    double y = y0 + (j + 0.5 - NUM_GROW) * dy;
    ...
    inteTotalEnergy.push_back(integratetotalenergy(u,uRigidBody));
    {
        ofstream outfile("/CFD_file/Tokamak_application/tungstenFirstDC/tokamak_t.dat");
        if (!outfile)
            return -1;
    ...
    {
        ofstream outfile("/CFD_file/Tokamak_application/tungstenFirstDC/integrated_totalEnergy.dat");
        if (!outfile)
            return -1;

        for (int i=0;i<inteTotalEnergy.size();i++){
            outfile << i*checkpointStep<<" "
                    << get<0>(inteTotalEnergy[i])<<" "//local
                    << get<1>(inteTotalEnergy[i])<<" "//global
                    << endl;
        }

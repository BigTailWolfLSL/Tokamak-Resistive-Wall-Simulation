...
////FirstVersion:The boundary condition for Perfect conducting wall with no ghost fluid way.
...
//                }
//
//                vector<tuple<double,double*>> minus_vn_p=multiplier(vn_ip,-1);
//                vector<tuple<double,double*>> vt_p=multiplier(vt_ip,slipOrNot);//TODO:在noslip的时候，应该要考虑Ut
//                tuple<vector<tuple<double,double*>>,vector<tuple<double,double*>>> minus_vn_p_pair= seperate(minus_vn_p,nx,ny);
//                tuple<vector<tuple<double,double*>>,vector<tuple<double,double*>>> vt_p_pair= seperate(vt_p,ny,-nx);
//                tuple<vector<tuple<double,double*>>,vector<tuple<double,double*>>> twoTimes_Un_pair= seperate(UnD,2*nx,2*ny);
//
//                Weight[i][j][k]= addition(get<0>(minus_vn_p_pair),get<0>(vt_p_pair));
//                Weight[i][j][k]= addition(Weight[i][j][k],get<0>(twoTimes_Un_pair));
//                Weight[i][j][k+1]= addition(get<1>(minus_vn_p_pair),get<1>(vt_p_pair));
//                Weight[i][j][k+1]= addition(Weight[i][j][k+1],get<1>(twoTimes_Un_pair));
//            }
//            else if (k==2||k==6){
//                continue;
//            }
...
//}
//void getWeight(vector<vector<Equ>> &U,vector<vector<Equ>> &URigidBody,int i,int j){
////SecondVersion:The boundary condition for resistive wall the first version with Neumann B_tangential, B_z and Dirichlet B_normal.
//    double slipOrNot=1.0;//1 for slip;-1 for no-slip
//
//    if (get<0>(Phi[i][j])<0) {
//        Normal cell=Normal(i,j)-Normal(2*get<0>(Phi[i][j])*get<1>(Phi[i][j])(0)/SimMesh.dx,2*get<0>(Phi[i][j])*get<1>(Phi[i][j])(1)/SimMesh.dy);
...
//}
//void getWeight(vector<vector<Equ>> &U,vector<vector<Equ>> &URigidBody,int i,int j){
////ThirdVersion:The boundary condition for resistive wall rho,momentums,total energy same，B field copy.
//    double slipOrNot=1.0;//1 for slip;-1 for no-slip
//
//    if (get<0>(Phi[i][j])<0) {
//        Normal cell=Normal(i,j)-Normal(2*get<0>(Phi[i][j])*get<1>(Phi[i][j])(0)/SimMesh.dx,2*get<0>(Phi[i][j])*get<1>(Phi[i][j])(1)/SimMesh.dy);
...
//}
//void getWeight(vector<vector<Equ>> &U,vector<vector<Equ>> &URigidBody,int i,int j){
////ForthVersion:The boundary condition for B_normal intermediate between Dirichlet and Neumann
//    double slipOrNot=1.0;//1 for slip;-1 for no-slip
//
//    if (get<0>(Phi[i][j])<0) {
//        Normal cell=Normal(i,j)-Normal(2*get<0>(Phi[i][j])*get<1>(Phi[i][j])(0)/SimMesh.dx,2*get<0>(Phi[i][j])*get<1>(Phi[i][j])(1)/SimMesh.dy);
...
//
//                vector<tuple<double,double*>> vt_p=multiplier(vt_ip,slipOrNot);//TODO:在noslip的时候，应该要考虑Ut
//                vector<tuple<double,double*>> vnD_p= congregate(UnD,2*exp(-rigidBodyBC_eta),vn_ip,-1*exp(-rigidBodyBC_eta));
//                vector<tuple<double,double*>> vnN_p= multiplier(vn_ip,(1- exp(-rigidBodyBC_eta)));
//                tuple<vector<tuple<double,double*>>,vector<tuple<double,double*>>> vt_p_pair= seperate(vt_p,ny,-nx);
//                tuple<vector<tuple<double,double*>>,vector<tuple<double,double*>>> vnD_p_pair= seperate(vnD_p,nx,ny);
//                tuple<vector<tuple<double,double*>>,vector<tuple<double,double*>>> vnN_p_pair= seperate(vnN_p,nx,ny);
//
//                Weight[i][j][k]= addition(get<0>(vnD_p_pair),get<0>(vt_p_pair));
//                Weight[i][j][k]= addition(Weight[i][j][k],get<0>(vnN_p_pair));
//                Weight[i][j][k+1]= addition(get<1>(vnD_p_pair),get<1>(vt_p_pair));
//                Weight[i][j][k+1]= addition(Weight[i][j][k+1],get<1>(vnN_p_pair));
//            }
//            else if (k==2||k==6){
//                continue;
//            }
...
//                vector<tuple<double,double*>> vy;vy.push_back(make_tuple(1,&U[i][j][k+1]));
//                vector<tuple<double,double*>> vt= congregate(vx,ny,vy,-nx);
//                vector<tuple<double,double*>> vn= congregate(vx,nx,vy,ny);
//                tuple<vector<tuple<double,double*>>,vector<tuple<double,double*>>> Un_pair= seperate(Un,exp(-rigidBodyBC_eta)*nx,exp(-rigidBodyBC_eta)*ny);
//                tuple<vector<tuple<double,double*>>,vector<tuple<double,double*>>> vn_pair= seperate(vn,(1-exp(-rigidBodyBC_eta))*nx,(1-exp(-rigidBodyBC_eta))*ny);
//                tuple<vector<tuple<double,double*>>,vector<tuple<double,double*>>> vt_pair= seperate(vt,ny,-nx);
//
//                Weight[i][j][k]= addition(get<0>(Un_pair), multiplier(get<0>(vt_pair),(slipOrNot+1.0)/2.0));//TODO:应该定义Ut并且根据它来得到，noslip是2Ut-vt，slip是vt
//                Weight[i][j][k]= addition(get<0>(vn_pair),Weight[i][j][k]);
//                Weight[i][j][k+1]= addition(get<1>(Un_pair), multiplier(get<1>(vt_pair),(slipOrNot+1.0)/2.0));
//                Weight[i][j][k+1]= addition(get<1>(vn_pair),Weight[i][j][k+1]);
//            } else {
//                Weight[i][j][k].push_back(make_tuple(1,&U[i][j][k]));
//            }
//        }
